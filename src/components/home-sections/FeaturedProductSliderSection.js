...
export default function FeaturedProductSliderSection({ sec, featuredProducts = [] }) {
    const { title, subtitle, badge_text, settings = {} } = sec;

    const featLimit = settings.limit || 10;
    const featItems = (featuredProducts || []).slice(0, featLimit);

    if (featItems.length === 0) return null;

    return (
        <section style={{ padding: '5rem 2rem 6rem', maxWidth: '1400px', margin: '0 auto', background: 'transparent' }}>
            <SectionDivider
                badge={badge_text || 'CURATED PICKS'}
                title={title || 'Featured Handloom Sarees'}
                subtitle={subtitle || 'Handpicked festive drapes, rich zari borders, and soft bridal silk creations.'}
                badgeType="gold"
            />
            ...
            <Swiper
                ...
                spaceBetween={24}
                slidesPerView={1}
                loop={featItems.length > 4}
                autoplay={{ delay: settings.auto_play_delay || 4000, disableOnInteraction: false }}
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
                                    <span style={{ color: '#7c8675', textTransform: 'uppercase', fontSize: '0.72rem', letterSpacing: '0.12em', fontWeight: 800 }}>
                                        {product.category || 'Featured Silk'}
                                    </span>
