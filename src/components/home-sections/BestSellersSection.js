...
export default function BestSellersSection({ sec, featuredProducts = [] }) {
    const { title, subtitle, badge_text, settings = {} } = sec;
    const [sliderIndex, setSliderIndex] = useState(0);
    const { wishlist = [], toggleWishlist } = useShop ? useShop() : {};

    const bestLimit = settings.limit || 8;
    const bestList = (featuredProducts || []).slice(0, bestLimit);

    useEffect(() => {
        if (bestList.length <= 4) return;
        const interval = setInterval(() => {
            setSliderIndex(prev => (prev + 1) > bestList.length - 4 ? 0 : prev + 1);
        }, settings.scroll_interval || 5000);
        return () => clearInterval(interval);
    }, [bestList.length, settings.scroll_interval]);
    ...
    return (
        <section 
            style={{ 
                ...
                position: 'relative', 
                overflow: 'hidden', 
                background: 'radial-gradient(ellipse at 88% 20%, rgba(212, 122, 6, 0.05) 0%, transparent 55%), radial-gradient(ellipse at 12% 80%, rgba(160, 102, 80, 0.04) 0%, transparent 55%), #fdfbf7' 
            }}
        >
            ...
            <div style={{ maxWidth: '1400px', margin: '0 auto', position: 'relative', zIndex: 2 }}>
                <SectionDivider
                    badge={badge_text || 'ROYAL SELECTIONS'}
                    title={title || 'Trending Handloom Sarees'}
                    subtitle={subtitle || 'Curated weaves crafted for wedding celebrations, festive radiance, and timeless moments.'}
                    badgeType="gold"
                />
                ...
                <div style={{ position: 'relative', overflow: 'hidden', padding: '10px 4px' }}>
                    <div style={{
                        ...
                        transform: `translateX(-${sliderIndex * (100 / Math.min(4, bestList.length))}%)`,
                        gap: '2rem'
                    }}>
