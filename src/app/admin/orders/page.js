...
export default function OrdersPage() {
    ...
    const [notifyStatusPicker, setNotifyStatusPicker] = useState(null); // { channel: 'whatsapp' | 'email', targetPhone?: string, targetEmail?: string }
    ...
    const handleResendEmail = () => {
        ...
        // If both exist and are different, show contact picker
        if (bEmail && sEmail && bEmail !== sEmail) {
            setNotificationSelection({ type: 'email', billing: bEmail, shipping: sEmail });
        } else {
            // Only one unique email — go straight to status picker
            setNotifyStatusPicker({ channel: 'email', targets: [...new Set([bEmail, sEmail].filter(Boolean))] });
        }
    };
    ...
    const handleResendWhatsApp = () => {
        ...
        if (bPhone && sPhone && bPhone !== sPhone) {
            setNotificationSelection({ type: 'whatsapp', billing: bPhone, shipping: sPhone });
        } else {
            // Only one unique phone — go straight to status picker
            setNotifyStatusPicker({ channel: 'whatsapp', targets: [...new Set([bPhone, sPhone].filter(Boolean))] });
        }
    };

    // Opens status picker for a specific set of targets
    const openStatusPicker = (channel, targets) => {
        setNotificationSelection(null);
        setNotifyStatusPicker({ channel, targets: Array.isArray(targets) ? targets : [targets] });
    };

    // Called when user picks a status in the picker — sends to ALL targets
    const handleSendNotificationWithStatus = async (statusType) => {
        if (!selectedOrder || !notifyStatusPicker) return;
        const { channel, targets } = notifyStatusPicker;
        setNotifyStatusPicker(null);
        setLoading(true);
        try {
            const requests = targets.map(target =>
                fetch('/api/admin/send-order-notification', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ...
                        targetPhone: channel === 'whatsapp' ? target : undefined,
                        targetEmail: channel === 'email' ? target : undefined,
                    })
                })
            );
            await Promise.all(requests);
            const label = channel === 'whatsapp' ? 'WhatsApp' : 'Email';
            setNotification({ message: `✅ "${statusType}" ${label} sent to: ${targets.join(', ')}`, type: 'success' });
        } catch (err) {
    ...
    return (
        <>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                                {orderItems.map((item, idx) => (
                                                    <div key={idx} style={{ display: 'flex', gap: '1.5rem', background: '#ffffff', padding: '1rem', borderRadius: '12px', border: `1px solid ${isEditingItems ? 'hsl(var(--primary) / 0.4)' : 'hsl(var(--border-subtle))'}` }}>
                                                        <div style={{ width: '100px', height: '130px', borderRadius: '8px', overflow: 'hidden', background: '#f1f5f9', border: '1px solid hsl(var(--border-subtle))', cursor: 'pointer' }} onClick={() => setZoomedImage(item.products?.image_url)}>
                                                            ...
                                                            {isEditingItems ? (
                                                                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginTop: 'auto', flexWrap: 'wrap' }}>
                                                                    <div>
                                                                        ...
                                                                        <input type="number" min="1" value={item.quantity} onKeyDown={(e) => { if (['e', 'E', '+', '-', '.'].includes(e.key)) e.preventDefault(); }}
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <input type="number" min="0" value={item.price_at_time} onKeyDown={(e) => { if (['e', 'E', '+', '-'].includes(e.key)) e.preventDefault(); }}
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                            ...
                                            <div className="card-sub" style={{ padding: '1.25rem', background: '#ffffff', borderRadius: '12px', border: '1px solid hsl(var(--border-subtle))' }}>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.5rem' }}>
                                                    ...
                                                    {/* Notification Selection Modal (Billing / Shipping / Both) */}
                                                    {notificationSelection && (
                                                        <div className="animate-enter" style={{ marginTop: '0.75rem', padding: '1rem', background: '#fff', borderRadius: '12px', border: '1px solid hsl(var(--border-subtle))', display: 'flex', flexDirection: 'column', gap: '0.5rem', boxShadow: '0 4px 15px rgba(0,0,0,0.08)' }}>
                                                            <div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'hsl(var(--text-muted))', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                                {notificationSelection.type === 'whatsapp' ? <MessageCircle size={13} color="#10b981" /> : <Mail size={13} color="hsl(var(--primary))" />}
                                                                Select Recipient
                                                            </div>

                                                            {/* Billing only */}
                                                            <button
                                                                onClick={() => openStatusPicker(notificationSelection.type, notificationSelection.billing)}
                                                                className="btn btn-secondary"
                                                                style={{ fontSize: '0.8rem', justifyContent: 'flex-start', padding: '0.6rem 1rem', wordBreak: 'break-all', height: 'auto', textAlign: 'left' }}
                                                            >
                                                                <span style={{ fontWeight: 800, fontSize: '0.7rem', textTransform: 'uppercase', marginRight: '6px', color: 'hsl(var(--text-muted))' }}>Billing</span>
                                                                {notificationSelection.billing}
                                                            </button>

                                                            {/* Shipping only */}
                                                            <button
                                                                onClick={() => openStatusPicker(notificationSelection.type, notificationSelection.shipping)}
                                                                className="btn btn-secondary"
                                                                style={{ fontSize: '0.8rem', justifyContent: 'flex-start', padding: '0.6rem 1rem', wordBreak: 'break-all', height: 'auto', textAlign: 'left' }}
                                                            >
                                                                <span style={{ fontWeight: 800, fontSize: '0.7rem', textTransform: 'uppercase', marginRight: '6px', color: 'hsl(var(--text-muted))' }}>Shipping</span>
                                                                {notificationSelection.shipping}
                                                            </button>

                                                            {/* Both */}
                                                            <button
                                                                onClick={() => openStatusPicker(notificationSelection.type, [notificationSelection.billing, notificationSelection.shipping])}
                                                                className="btn"
                                                                style={{
                                                                    fontSize: '0.8rem', padding: '0.6rem 1rem', fontWeight: 700,
                                                                    background: notificationSelection.type === 'whatsapp' ? '#e8fff3' : '#f0f9ff',
                                                                    color: notificationSelection.type === 'whatsapp' ? '#10b981' : 'hsl(var(--primary))',
                                                                    border: `1px solid ${notificationSelection.type === 'whatsapp' ? 'rgba(16,185,129,0.3)' : 'hsl(var(--primary) / 0.3)'}`,
                                                                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px'
                                                                }}
                                                            >
                                                                {notificationSelection.type === 'whatsapp' ? <MessageCircle size={14} /> : <Mail size={14} />}
                                                                Both (Billing + Shipping)
                                                            </button>

                                                            <button onClick={() => setNotificationSelection(null)} className="btn" style={{ fontSize: '0.78rem', background: '#f1f5f9', color: 'hsl(var(--text-muted))' }}>Cancel</button>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                    ...
                                    <div style={{ flex: 1, overflow: 'auto', padding: '2rem' }}>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                                            ...
                                            <div>
                                                ...
                                                <input type="tel" placeholder="91..." value={newOrder.billing_phone} onChange={e => setNewOrder({ ...newOrder, billing_phone: e.target.value.replace(/[^0-9]/g, '').slice(0, 10) })} pattern="[0-9]{10}" maxLength="10" minLength="10" required style={{ width: '100%', padding: '0.85rem', borderRadius: '10px', background: '#f1f5f9', border: '1px solid hsl(var(--border-subtle))', color: 'hsl(var(--text-main))' }} />
                                            </div>
                                            ...
                                            {!newOrder.same_as_billing && (
                                                <>
                                                    ...
                                                    <div>
                                                        ...
                                                        <input type="tel" placeholder="91..." value={newOrder.shipping_phone} onChange={e => setNewOrder({ ...newOrder, shipping_phone: e.target.value.replace(/[^0-9]/g, '').slice(0, 10) })} pattern="[0-9]{10}" maxLength="10" minLength="10" style={{ width: '100%', padding: '0.85rem', borderRadius: '10px', background: '#f1f5f9', border: '1px solid hsl(var(--border-subtle))', color: 'hsl(var(--text-main))' }} />
                                                    </div>
                                                </>
                                            )}
                                                ...
                                                {newOrder.items.map((item, idx) => (
                                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: '#ffffff', padding: '0.75rem 1rem', borderRadius: '10px', border: '1px solid hsl(var(--border-subtle))' }}>
                                                        ...
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                            <input type="number" min="1" value={item.quantity} onKeyDown={(e) => { if (['e', 'E', '+', '-', '.'].includes(e.key)) e.preventDefault(); }} onChange={e => {
            ...
            {notifyStatusPicker && (
                ...
                }} onClick={() => setNotifyStatusPicker(null)}>
                    <div style={{
                        background: 'hsl(var(--bg-card))', borderRadius: '24px', width: '100%', maxWidth: '460px',
                        padding: '2rem', boxShadow: '0 30px 80px rgba(0,0,0,0.4)',
                        border: '1px solid hsl(var(--border-subtle))', animation: 'fadeIn 0.2s ease'
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                            <div style={{
                                ...
                                {notifyStatusPicker.channel === 'whatsapp' ? <MessageCircle size={22} /> : <Mail size={22} />}
                            </div>
                            <div>
                                <div style={{ fontWeight: 800, fontSize: '1.1rem', color: 'hsl(var(--text-main))' }}>
                                    Send {notifyStatusPicker.channel === 'whatsapp' ? 'WhatsApp' : 'Email'} Notification
                                </div>
                                <div style={{ fontSize: '0.75rem', color: 'hsl(var(--text-muted))', marginTop: '2px' }}>
                                    Will send to {notifyStatusPicker.targets?.length || 1} contact(s)
                                </div>
                            </div>
                        </div>
                        {/* Action Buttons */}
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                            {[
                                { status: 'PLACED', label: 'Order Placed', desc: 'Order confirmed & being prepared', color: '#f59e0b', bg: '#fffbeb' },
                                { status: 'AWAITING_PAYMENT', label: 'Awaiting Payment', desc: 'Payment pending from customer', color: '#f97316', bg: '#fff7ed' },
                                { status: 'PAID', label: 'Payment Received', desc: 'Payment confirmed with invoice', color: '#10b981', bg: '#ecfdf5' },
                                { status: 'PACKING', label: 'Order Packing', desc: 'Currently being packed for shipping', color: '#6366f1', bg: '#eef2ff' },
                                { status: 'SHIPPED', label: 'Order Shipped', desc: 'Dispatched with tracking info', color: '#0ea5e9', bg: '#f0f9ff' },
                                { status: 'DELIVERED', label: 'Order Delivered', desc: 'Successfully delivered to customer', color: '#10b981', bg: '#ecfdf5' },
                                { status: 'CANCELLED', label: 'Order Cancelled', desc: 'Order has been cancelled', color: '#ef4444', bg: '#fef2f2' },
                            ].map(({ status, label, desc, color, bg }) => (
                                <button
                                    key={status}
                                    onClick={() => handleSendNotificationWithStatus(status)}
                                    style={{
                                ...
                                >
                                    <div>
                                        ...
                                        <div style={{ fontSize: '0.73rem', color: '#64748b', marginTop: '2px' }}>{desc}</div>
                                    </div>
                                    <div style={{ fontSize: '0.7rem', fontWeight: 800, background: color, color: 'white', padding: '3px 8px', borderRadius: '6px', flexShrink: 0, marginLeft: '0.5rem' }}>
                                        SEND
                                    </div>
                                </button>
                            ))}
                        ...
                        <button onClick={() => setNotifyStatusPicker(null)} style={{
                            ...
                            borderRadius: '12px', color: 'hsl(var(--text-muted))', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem'
                        }}>
                            Cancel
                        </button>
                    </div>
