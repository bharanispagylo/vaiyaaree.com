...
export async function GET(req) {
    try {
        ...
        let autoBackupConfig = { enabled: false, frequency: 'daily' };
        if (autoSetting?.value) {
            try {
                autoBackupConfig = typeof autoSetting.value === 'string' ? JSON.parse(autoSetting.value) : autoSetting.value;
            } catch (e) {}
        }
...
export async function POST(req) {
    try {
        ...
        const { action = 'create', id, format = 'JSON', dateRange = 'all', startDate, endDate, status = 'ALL', paymentStatus = 'ALL', source = 'ALL', notes = '', autoBackupConfig } = body;
        ...
        // ─────────────────────────────────────────────────────────────────────────────
        // ACTION 2: TOGGLE AUTOMATED BACKUPS
        // ─────────────────────────────────────────────────────────────────────────────
        if (action === 'toggle-auto') {
            const configPayload = JSON.stringify(autoBackupConfig || { enabled: false, frequency: 'daily' });
            await mysqlClient.from('app_settings').upsert({
                key: 'auto_order_backup',
                value: configPayload,
                updated_at: new Date().toISOString()
            });
            return NextResponse.json({ success: true, message: 'Automated backup settings updated' });
        }
        ...
        // ─────────────────────────────────────────────────────────────────────────────
        // ACTION 4: CREATE NEW BACKUP SNAPSHOT
        // ─────────────────────────────────────────────────────────────────────────────
        let sqlWhere = ['1=1'];
        let params = [];
        ...
        const filtersJson = JSON.stringify({ dateRange, startDate, endDate, status, paymentStatus, source, format, type: 'MANUAL', createdBy: 'Admin' });
        ...
        await pool.query(
            ...
            [
                backupId, filename, 'MANUAL', format, totalOrdersCount, totalRevenue,
                rangeLabel, fileSizeBytes, backupContent, filtersJson,
                'COMPLETED', notes || null, 'Admin'
            ]
        );

        return NextResponse.json({
            success: true,
            message: `Order backup ${backupId} generated successfully!`,
            backup: {
                id: backupId,
                filename,
                format,
